...
public class PushSmart : MonoBehaviour
{
    ...
    protected virtual void HandlePush()
    {
        ...
        else if (IsSitDown() && player.GetNextLocation() == newPos)
        {
            MoveDown();
        }
    }
    ...
    {
        if (BoxManager.allBoxLocation.ContainsKey(begin) && (BoxManager.allBoxLocation.ContainsKey(last) || player.GetNextLocation() == last || TilemapManager.wallIsBlocked.ContainsKey(last)))
        {
            return false;
        }
